...
export function PdfControlBar({
  ...
  return (
    <div className="relative px-2 py-2 sm:px-4 sm:py-3 bg-card border-t border-border z-10 flex items-center justify-between gap-1.5 sm:gap-2 transition-colors duration-200 shrink-0">
      {/* Page Navigation */}
      <div className="flex items-center gap-1 sm:gap-1.5 bg-muted rounded-md p-0.5 shrink-0">
        <button
          onClick={() => onPageChange(Math.max(1, currentPage - 1))}
          disabled={currentPage <= 1}
          className="p-1 sm:p-1.5 rounded hover:bg-foreground/20 text-foreground transition-colors disabled:opacity-40"
          title={m.previous_page()}
        >
          <ChevronLeft size={16} />
        </button>
        <span className="px-1 sm:px-2 text-xs font-mono font-medium text-foreground select-none whitespace-nowrap">
      ...
      <div className="flex items-center gap-1 sm:gap-2 shrink-0">
        <div className="flex items-center gap-0.5 sm:gap-1 bg-muted rounded-md p-0.5">
          <button
            onClick={onZoomOut}
            className="p-1 sm:p-1.5 rounded hover:bg-foreground/20 text-foreground transition-colors"
            title={m.zoom_out()}
          >
            <Minus size={16} />
          </button>
          <span className="w-10 sm:w-12 text-center text-xs font-mono font-medium text-foreground select-none">
            {Math.round(zoom * 100)}%
          </span>
          <button
            onClick={onZoomIn}
        ...
        <button
          onClick={onFit}
          className="text-xs font-medium px-2 py-1 sm:px-3 sm:py-1.5 rounded bg-muted hover:bg-foreground/20 text-foreground transition-colors border border-transparent"
        >
          {m.fit()}
        </button>
        {allowDownload && (
